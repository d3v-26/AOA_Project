...
/**
 *   DP Memoization implementation of ALG8. Its time complexity is in order of m*n.
...
int DPMemoizationMN(vector<vector<int>> &A, vector<vector<int>> &DiffSoFar, vector<vector<int>> &DP, vector<vector<int>> &Buy, int M, int N, int c)
{
    ...
    int previousDayProfit = DPMemoizationMN(A, DiffSoFar, DP, Buy, M, N-1, c);
    
    // What is previous stock's profit?
    int previousStockProfit = M <= 0 ? 0 : DPMemoizationMN(A, DiffSoFar, DP, Buy, M-1, N, c);
    
    // What is profit if we buy? i.e. A[M][N] - A[M][N-1] + profit at day - c - 2 i.e. before cooldown
    int profitIfWeBuy = DPMemoizationMN(A, DiffSoFar, DP, Buy, m-1, max(0, N-c-2), c);
